...
export async function onRequestGet({ env }) {
  try {
    ...
    const [prizeResult, itemResult] = await Promise.all([
      ...
      env.DB.prepare("SELECT id, campaign_id, prize_category_id, name, sort_order FROM prize_items WHERE active = 1 ORDER BY campaign_id, prize_category_id, sort_order, id").all(),
    ]);
    ...
    return json({
      items: campaigns.map((row) => ({
        ...
        prizeItems: prizeItems.filter((item) => item.campaign_id === row.id).map((item) => ({ id: item.id, prizeCategoryId: item.prize_category_id, name: item.name, sortOrder: item.sort_order })),
      })),
