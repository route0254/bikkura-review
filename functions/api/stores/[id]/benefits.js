...
export async function onRequestGet({ request, env, params }) {
  try {
    ...
    const rows = (await env.DB.prepare(`
      SELECT benefit.*, latest.availability, latest.observed_at,
        (SELECT COUNT(*) FROM benefit_reports r WHERE r.benefit_id = benefit.id AND r.store_id = ? AND r.status = 'active' AND r.observed_at BETWEEN ? AND ? AND r.availability = 'available') AS available_count,
        (SELECT COUNT(*) FROM benefit_reports r WHERE r.benefit_id = benefit.id AND r.store_id = ? AND r.status = 'active' AND r.observed_at BETWEEN ? AND ? AND r.availability = 'unavailable') AS unavailable_count,
        (SELECT COUNT(*) FROM benefit_reports r WHERE r.benefit_id = benefit.id AND r.store_id = ? AND r.status = 'active' AND r.observed_at BETWEEN ? AND ? AND r.availability = 'unknown') AS unknown_count
      FROM benefit_campaigns benefit
      LEFT JOIN benefit_reports latest ON latest.id = (
        SELECT id FROM benefit_reports WHERE benefit_id = benefit.id AND store_id = ? AND status = 'active' AND observed_at <= ?
    ...
    return json({ campaign: mapCampaign(campaign), asOf: now.toISOString(), items: rows.map((row) => ({
      ...
      conditions: row.conditions, sourceUrl: row.source_url,
      latest: row.observed_at ? { availability: row.availability, observedAt: row.observed_at, freshness: benefitFreshness(row.observed_at, now) } : null,
