...
export function sortStores(stores, mode = "default", origin = null) {
  ...
  const discoveryCount = (store) => Number(stats(store).reportCount ?? 0) + Number(stats(store).externalCollectionCount ?? 0);
  sorted.sort((left, right) => {
    if (mode === "default") return discoveryCount(right) - discoveryCount(left);
