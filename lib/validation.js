...
export function validateReportPayload(payload, context = {}) {
  ...
  if (!Array.isArray(payload.prizes)) errors.push("景品データの形式が正しくありません。");
    ...
    for (const prize of payload.prizes) {
      ...
      if (seen.has(prize?.prizeCategoryId)) errors.push("同じ景品種類が重複しています。");
      seen.add(prize?.prizeCategoryId);
    }
  }
  ...
  if (!Array.isArray(itemBreakdowns)) errors.push("個別景品データの形式が正しくありません。");
    ...
    const categoryQuantities = new Map((payload.prizes ?? []).map((prize) => [prize.prizeCategoryId, prize.quantity]));
    ...
    for (const breakdown of itemBreakdowns) {
      ...
      if (seenCategories.has(categoryId)) errors.push("同じ個別景品カテゴリが重複しています。");
      ...
      const categoryQuantity = categoryQuantities.get(categoryId) ?? 0;
