...
export function validateReportPayload(payload, context = {}) {
  ...
  if (valuesAreNumbers && prizeValuesAreNumbers) {
    ...
    const totalPrizes = payload.prizes.reduce((sum, prize) => sum + prize.quantity, 0) + payload.unknownPrizeCount;
    if (totalPrizes > totalWins) errors.push("景品個数の合計は、当たり回数の合計以下にしてください。");
  }
