import { USER_ACQUISITION_TYPES, acquisitionTypeOf } from "./report-policy.js";
...
const ACQUISITION_TYPE_SET = new Set(USER_ACQUISITION_TYPES);
...
export function validateReportPayload(payload, context = {}) {
  ...
  if (!Array.isArray(payload.prizes)) errors.push("景品データの形式が正しくありません。");
    ...
    for (const prize of payload.prizes) {
      ...
      if (!ACQUISITION_TYPE_SET.has(acquisitionType)) errors.push("景品の取得経路が正しくありません。");
  ...
  if (!Array.isArray(itemBreakdowns)) errors.push("個別景品データの形式が正しくありません。");
    ...
    for (const breakdown of itemBreakdowns) {
      ...
      if (!ACQUISITION_TYPE_SET.has(acquisitionType)) errors.push("個別景品の取得経路が正しくありません。");
  ...
  if (valuesAreNumbers && prizeValuesAreNumbers) {
    ...
    const categorizedPrizes = payload.prizes.filter((prize) => acquisitionTypeOf(prize) === "draw").reduce((sum, prize) => sum + prize.quantity, 0);
    ...
    if (payload.prizeBreakdownStatus === "complete") {
      ...
      if (categorizedPrizes !== totalWins) errors.push("すべて入力した場合、景品個数の合計を当たり回数の合計と一致させてください。");
    } else if (totalPrizes > totalWins) {
      errors.push("景品個数の合計は、当たり回数の合計以下にしてください。");
    }
  }
