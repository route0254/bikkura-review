import { acquisitionTypeOf, prizeInputModeOf, resultInputModeOf } from "./report-policy.js";
import { normalizeGoodsPayload } from "./goods.js";

export const REPORT_LIMITS = Object.freeze({
  maxCount: 300,
  maxTotalDraws: 300,
  maxSpendAmountYen: 1_000_000,
  maxBodyBytes: 16_384,
...
const RESULT_INPUT_MODES = new Set(["detailed", "simple"]);
...
export function validateReportPayload(payload, context = {}) {
  ...
  if (payload.goodsInput === true) {
    const normalized = normalizeGoodsPayload(payload, context);
    return [...new Set([...normalized.errors, ...validateReportPayload(normalized.validationPayload, context)])];
  }
  ...
  if (!RESULT_INPUT_MODES.has(resultInputMode)) errors.push("結果の入力方法が正しくありません。");
  const simpleReport = resultInputMode === "simple";
  ...
  const spendAmountYen = payload.spendAmountYen ?? null;
  const reportedTotalDraws = payload.reportedTotalDraws ?? null;
  const reportedPrizeCount = payload.reportedPrizeCount ?? null;
  const simpleGuaranteedPrizeCount = payload.simpleGuaranteedPrizeCount ?? null;
  if (simpleReport) {
    if (!Number.isInteger(spendAmountYen) || spendAmountYen < 1 || spendAmountYen > REPORT_LIMITS.maxSpendAmountYen) errors.push("おおよその利用金額を1〜1,000,000円の整数で入力してください。");
    if (reportedTotalDraws !== null && !isBoundedInteger(reportedTotalDraws)) errors.push(`抽選回数は空欄、または0〜${REPORT_LIMITS.maxTotalDraws}の整数で入力してください。`);
    if (!isBoundedInteger(reportedPrizeCount)) errors.push(`もらった景品の合計は0〜${REPORT_LIMITS.maxCount}の整数で入力してください。`);
    if (prizeInputMode !== "total") errors.push("かんたん入力の景品入力方法が正しくありません。");
    if (payload.usageType !== "unknown") errors.push("かんたん入力では利用区分を不明として送信してください。");
    if (guaranteedPrizeCount !== 0) errors.push("かんたん入力の確定景品は専用項目へ入力してください。");
    if (simpleGuaranteedPrizeCount !== null && (!isBoundedInteger(simpleGuaranteedPrizeCount) || simpleGuaranteedPrizeCount > reportedPrizeCount)) errors.push("確定でもらった景品は空欄、または景品合計以下の0以上の整数にしてください。");
  } else if (spendAmountYen !== null || reportedTotalDraws !== null || reportedPrizeCount !== null || simpleGuaranteedPrizeCount !== null) {
    errors.push("詳しく入力では、かんたん入力専用の項目を送信しないでください。");
  }
  ...
  if (simpleReport && pairs.some(([draws, wins]) => draws !== 0 || wins !== 0)) errors.push("かんたん入力ではパネル・スマホ別の回数を送信しないでください。");
  ...
  if (valuesAreNumbers && prizeValuesAreNumbers && simpleReport) {
    const categorizedPrizes = payload.prizes.reduce((sum, prize) => sum + prize.quantity, 0);
    const totalPrizes = categorizedPrizes + payload.unknownPrizeCount;
    if (Number.isInteger(reportedPrizeCount) && totalPrizes !== reportedPrizeCount) errors.push("景品の種類別個数と種類不明の個数を、もらった景品の合計に合わせてください。");
    if (payload.prizeBreakdownStatus === "complete" && payload.unknownPrizeCount !== 0) errors.push("内訳がすべて分かる場合、種類不明の景品は0個にしてください。");
    if (payload.prizeBreakdownStatus === "unknown" && categorizedPrizes !== 0) errors.push("内訳が分からない場合、景品の種類別個数は0個にしてください。");
    if (payload.prizeBreakdownStatus === "partial" && (categorizedPrizes === 0 || payload.unknownPrizeCount === 0)) errors.push("一部分かる場合、分かる景品と種類不明の景品をそれぞれ1個以上にしてください。");
  } else if (valuesAreNumbers && prizeValuesAreNumbers) {
