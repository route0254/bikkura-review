...
export function spendStatsSql(where) {
  return `WITH base AS (
    SELECT spend_amount_yen AS spend, reported_prize_count AS total,
      reference_guaranteed_count AS guaranteed,
      reported_prize_count - reference_guaranteed_count AS drawn,
    ...
    FROM active_spend_reports WHERE ${where} AND spend_amount_yen > 0 AND reported_prize_count IS NOT NULL
  ), metrics AS (
