...
export function filterPublicExternalReports(reports = []) {
  return reports.filter((report) => report.status === "active" && report.storeId && report.externalUrl);
}
...
export function validateExternalReports(reports, context = {}) {
  ...
  for (const report of reports) {
    ...
    if (report.status === "active" && !report.externalUrl) errors.push(`${label}: activeにするにはexternalUrlが必要です`);
