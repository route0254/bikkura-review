...
export function assessReportRisk(payload, context = {}) {
  const totalDraws = Number(payload.panelDraws) + Number(payload.mobileDraws);
  const totalWins = Number(payload.panelWins) + Number(payload.mobileWins);
  ...
  if (totalDraws >= 120 && totalWins === 0) { score += 15; reasons.push("large_zero_win_report"); }
