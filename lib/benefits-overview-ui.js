...
let api,master=[],selected=null,offset=0,sequence=0,teaserSequence=0,masterCampaign=null,teaserLoading=null;
...
export function loadBenefitTeaser(campaign) {
  if(teaserLoading?.campaignId===campaign.id)return teaserLoading.promise;
  const promise=fetchBenefitTeaser(campaign);
  teaserLoading={campaignId:campaign.id,promise};
  return promise.finally(()=>{if(teaserLoading?.promise===promise)teaserLoading=null;});
}
async function fetchBenefitTeaser(campaign) {
