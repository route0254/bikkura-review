...
export function initializeBenefitUI(dependencies) {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (!benefit) return;
    lastTrigger = button;
    selection = { ...current, benefit };
    submitted = false;
    form.reset();
    $("#benefit-store-name").textContent = current.store.name;
    $("#benefit-name").textContent = benefit.name;
    $("#benefit-observed-at").value = localTime();
    $("#benefit-observed-at").max = localTime();
    $("#benefit-errors").hidden = true;
    $("#benefit-form-status").textContent = "";
    $("#benefit-submit").disabled = false;
    dialog.showModal();
    ensureBenefitTurnstile(true);
  });
